...
class Engineer:
    ...
    def __init__(self):
        self.policy = ForwardNet(self.__INPUT_SIZE, self.__HIDDEN_DEPTH, self.__HIDDEN_WIDTH, self.__OUTPUT_SIZE)
        # Load in the actor model saved by the PPO algorithm
        self.policy.load_state_dict(MODEL)
        self.dummy_agent = CatieAgent()

    def get_allocation(self, target_allocations, anti_target_allocations, is_target_choices):
        """
        ...
        """
        assignments = np.sum(target_allocations), np.sum(anti_target_allocations)
        assignments_left_ratio = [(25 - assignments[0]) / 25, (25 - assignments[1]) / 25]
        trial_number = len(target_allocations)
        self.update_agent_history(target_allocations, anti_target_allocations, is_target_choices)
        catie_params = self.dummy_agent.get_catie_param()
        observation = np.array([*catie_params, assignments[0] / 25, assignments[1] / 25,
                                *assignments_left_ratio, trial_number / 100], dtype=np.float64)
        return self.policy(observation, is_test=True)

    def update_agent_history(self, target_allocations, anti_target_allocations, is_target_choices):
        self.dummy_agent = CatieAgent()
        for i, outcome in enumerate(zip(target_allocations, anti_target_allocations)):
            self.dummy_agent.receive_outcome(is_target_choices[i], outcome)
...
def allocate(target_allocations, anti_target_allocations, is_target_choices):
    ...
    :return: A two-element tuple where the first binary element indicates
                ...
                output function (see below in this file)
    """
    action_probs = engineer.get_allocation(target_allocations, anti_target_allocations, is_target_choices)
    action = engineer.INDEX_TO_ACTION[torch.multinomial(action_probs, num_samples=1).item()]
    return constrain(target_allocations, action[0]), constrain(anti_target_allocations, action[1])
